...
export default function MakeVideoTwirl({ videoId, auth }: Props) {
                ...
                for (let x = 0; x < 640; x++) {
                    ...
                    if (frame2.data[index] == 0 || frame2.data[index + 1] == 0 || frame2.data[index + 2] == 0 || frame2.data[index + 3] == 0) {
                        ...
                        while ((frame2.data[tempIndex2] == 0 || frame2.data[tempIndex2 + 1] == 0 || frame2.data[tempIndex2 + 2] == 0 || frame2.data[tempIndex2 + 3] == 0) && tempIndex2 < 1228800) // 1228800 = 640*480*4
                            tempIndex2 += 4
                        frame2.data[index] = Math.floor(frame2.data[tempIndex1] + frame2.data[tempIndex2]) / 2;
                        frame2.data[index + 1] = Math.floor(frame2.data[tempIndex1 + 1] + frame2.data[tempIndex2 + 1]) / 2;
                        frame2.data[index + 2] = Math.floor(frame2.data[tempIndex1 + 2] + frame2.data[tempIndex2 + 2]) / 2;
                        frame2.data[index + 3] = Math.floor(frame2.data[tempIndex1 + 3] + frame2.data[tempIndex2 + 3]) / 2;
                    }
                }
            }
    ...
    return (
        <div >
            <canvas id={`${videoId}_twirl`} width="640" height="480" ref={cloneRef} style={{ display: "none" }} ></canvas>
        </div >
    )
}
